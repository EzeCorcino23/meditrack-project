...
export default function Layout({
  ...
  const accentDark = accentColor === "#059669" ? "#065F46"
                   : accentColor === "#0891B2" ? "#0C4A6E"
                   : "#1E3A8A";
  const gradient = `linear-gradient(135deg, ${accentDark}, ${accentColor})`;

  const panelLabel = adminRole === "Administrador" ? "Panel Administrativo"
                   : adminRole === "Médico"         ? "Panel Médico"
                   : adminRole === "Recepción"      ? "Panel de Recepción"
                   : "Panel";
    ...
    <>
      ...
      <div style={{ position:"fixed", inset:0, zIndex:10, display:"flex" }}>
        ...
        <div className="sidebar-scroll"
          ...
          <div style={{ padding:"12px 10px", borderBottom:"1px solid rgba(30,58,138,0.1)", display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
            {!collapsed && (
              <div style={{ display:"flex", alignItems:"center", gap:9, flex:1, minWidth:0 }}>
                <div style={{ width:32, height:32, borderRadius:9, background:gradient, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:`0 3px 10px ${accentColor}66` }}>
                ...
                <div style={{ minWidth:0 }}>
                  ...
                  <p style={{ fontSize:10, color:"#6B7280", fontWeight:500, marginTop:1 }}>{panelLabel}</p>
                </div>
              </div>
            )}
            <button className="toggle-btn" onClick={() => setCollapsed(c => !c)} title={collapsed ? "Expandir" : "Colapsar"}
          ...
          {}
          {!collapsed && (
            <div style={{ margin:"10px 12px 0", padding:"8px 10px", background:`${accentColor}12`, borderRadius:10, border:`1px solid ${accentColor}28`, display:"flex", alignItems:"center", gap:8 }}>
          ...
          <nav style={{ padding:collapsed?"8px 0":"8px", flex:1, marginTop:6 }}>
            ...
            {navItems.map(item => (
              <button key={item.id} className="nav-item"
                ...
                style={{ width:"100%", display:"flex", alignItems:"center", gap:collapsed?0:9, justifyContent:collapsed?"center":"flex-start", padding:collapsed?"9px 0":"8px 10px", borderRadius:9, border:"none", cursor:"pointer", background:section===item.id?`${accentColor}18`:"transparent", marginBottom:2, transition:"background 0.15s", textAlign:"left", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
                <div style={{ width:30, height:30, borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center", background:section===item.id?gradient:`${accentColor}14`, flexShrink:0, transition:"background 0.2s" }}>
                  <SvgIcon name={item.icon} size={13} color={section===item.id?"#fff":accentColor} />
                </div>
        ...
        <div className="content-scroll" style={{ flex:1, height:"100%", overflowY:"auto", padding:"18px 28px 28px" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              <div style={{ width:36, height:36, borderRadius:9, background:`${accentColor}20`, display:"flex", alignItems:"center", justifyContent:"center" }}>
                <SvgIcon name={navItems.find(n=>n.id===section)?.icon||"dashboard"} size={16} color={accentColor} />
              </div>
              <div>
                <p style={{ fontSize:10, color:"rgba(255,255,255,0.55)", fontWeight:500 }}>MediTrack · {adminRole}</p>
            ...
            <div style={{ display:"flex", gap:10, alignItems:"center" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8, padding:"5px 12px 5px 6px", background:"rgba(255,255,255,0.12)", backdropFilter:"blur(10px)", border:"1px solid rgba(255,255,255,0.2)", borderRadius:10, cursor:"pointer" }}>
                <div style={{ width:28, height:28, borderRadius:"50%", background:gradient, display:"flex", alignItems:"center", justifyContent:"center", border:"2px solid rgba(255,255,255,0.3)" }}>
        ...
        <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(0,0,0,0.35)", backdropFilter:"blur(6px)" }}>
          <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", background:"rgba(255,255,255,0.88)", backdropFilter:"blur(24px)", border:"1px solid rgba(255,255,255,0.6)", borderRadius:18, padding:"40px 44px", width:380, textAlign:"center", boxShadow:"0 20px 60px rgba(0,0,0,0.2)", animation:"popIn 0.3s cubic-bezier(.22,.68,0,1.2) forwards" }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setShowLogout(false)} style={{ flex:1, padding:12, borderRadius:9, border:"1.5px solid rgba(30,58,138,0.25)", background:"rgba(239,246,255,0.7)", color:"#1E3A8A", fontWeight:600, fontSize:14, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Cancelar</button>
              <button onClick={handleLogout} style={{ flex:1, padding:12, borderRadius:9, border:"none", background:"linear-gradient(135deg,#DC2626,#EF4444)", color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", fontFamily:"'Plus Jakarta Sans',sans-serif", boxShadow:"0 4px 14px rgba(220,38,38,0.35)" }}>Sí, salir</button>
            </div>
          </div>
        </div>
      )}
