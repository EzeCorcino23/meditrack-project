...
export default function ConsultationsSection() {
  ...
  const [form, setForm] = useState({ patient:"", date:"", reason:"", diagnosis:"", treatment:"" });

  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24 }}>
        <div><h2 style={sc.sectionTitle}>Consultas</h2><p style={sc.sectionSub}>Registro y seguimiento de consultas médicas</p></div>
        <button style={sc.primaryBtn} onClick={() => setShowModal(true)}><SvgIcon name="plus" size={14} color="#fff" /> Nueva Consulta</button>
      </div>
      <div style={sc.tableCard}>
        <table style={sc.table}>
          <thead><tr>{["Paciente","Fecha","Motivo","Diagnóstico","Tratamiento","Acciones"].map(h => <th key={h} style={sc.th}>{h}</th>)}</tr></thead>
          <tbody>{[1,2,3,4].map(i => <tr key={i} style={sc.tr}>{[100,80,100,100,100,68].map((w,j) => <td key={j} style={sc.td}><div style={{ height:13, borderRadius:6, background:"rgba(30,58,138,0.07)", width:w }} /></td>)}</tr>)}</tbody>
        </table>
      </div>
      ...
      {showModal && (
        <div style={ms.backdrop}>
          <div style={ms.modal}>
            <h3 style={ms.title}>Nueva Consulta</h3>
            ...
            <div style={ms.field}><label style={ms.label}>Motivo *</label><textarea style={{ ...ms.input, minHeight:60, resize:"vertical" }} placeholder="Motivo de consulta..." value={form.reason} onChange={e => setForm(f => ({ ...f, reason:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Diagnóstico</label><textarea style={{ ...ms.input, minHeight:60, resize:"vertical" }} placeholder="Diagnóstico..." value={form.diagnosis} onChange={e => setForm(f => ({ ...f, diagnosis:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Tratamiento</label><textarea style={{ ...ms.input, minHeight:60, resize:"vertical" }} placeholder="Tratamiento indicado..." value={form.treatment} onChange={e => setForm(f => ({ ...f, treatment:e.target.value }))} /></div>
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => setShowModal(false)} style={ms.cancelBtn}>Cancelar</button>
              <button style={ms.saveBtn}>Guardar Consulta</button>
            </div>
          </div>
        </div>
      )}
