  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24 }}>
        <div><h2 style={sc.sectionTitle}>Notas Clínicas</h2><p style={sc.sectionSub}>Anotaciones y observaciones por consulta</p></div>
        <button style={sc.primaryBtn} onClick={() => setShowModal(true)}><SvgIcon name="plus" size={14} color="#fff" /> Nueva Nota</button>
      </div>
      <div style={sc.tableCard}>
        <table style={sc.table}>
          <thead><tr>{["Paciente","Nota","Fecha","Acciones"].map(h => <th key={h} style={sc.th}>{h}</th>)}</tr></thead>
          <tbody>{[1,2,3,4].map(i => <tr key={i} style={sc.tr}>{[100,200,80,68].map((w,j) => <td key={j} style={sc.td}><div style={{ height:13, borderRadius:6, background:"rgba(30,58,138,0.07)", width:w }} /></td>)}</tr>)}</tbody>
        </table>
      </div>
      {showModal && (
        <div style={ms.backdrop}><div style={{ ...ms.modal, width:440 }}>
          <h3 style={ms.title}>Nueva Nota Clínica</h3>
          ...
          <div style={ms.field}><label style={ms.label}>Nota *</label><textarea style={{ ...ms.input, minHeight:100, resize:"vertical" }} placeholder="Escribe la nota clínica..." value={form.note} onChange={e => setForm(f => ({ ...f, note:e.target.value }))} /></div>
          <div style={{ display:"flex", gap:10 }}><button onClick={() => setShowModal(false)} style={ms.cancelBtn}>Cancelar</button><button style={ms.saveBtn}>Guardar Nota</button></div>
        </div></div>
      )}
    </div>
  );
