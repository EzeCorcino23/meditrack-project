...
export default function VitalsSection() {
  ...
  const [form, setForm] = useState({ patient:"", pressure:"", weight:"", temperature:"", pulse:"", height:"" });

  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24 }}>
        <div><h2 style={sc.sectionTitle}>Signos Vitales</h2><p style={sc.sectionSub}>Registro y seguimiento de signos vitales</p></div>
        <button style={sc.primaryBtn} onClick={() => setShowModal(true)}><SvgIcon name="plus" size={14} color="#fff" /> Registrar Signos</button>
      </div>
      <div style={sc.tableCard}>
        <table style={sc.table}>
          <thead><tr>{["Paciente","Presión","Peso","Temp.","Pulso","Talla","Fecha","Acciones"].map(h => <th key={h} style={sc.th}>{h}</th>)}</tr></thead>
          <tbody>{[1,2,3,4].map(i => <tr key={i} style={sc.tr}>{[100,60,50,50,50,50,80,68].map((w,j) => <td key={j} style={sc.td}><div style={{ height:13, borderRadius:6, background:"rgba(30,58,138,0.07)", width:w }} /></td>)}</tr>)}</tbody>
        </table>
      </div>
      {showModal && (
        <div style={ms.backdrop}><div style={ms.modal}>
          <h3 style={ms.title}>Registrar Signos Vitales</h3>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px 14px" }}>
            <div style={ms.field}><label style={ms.label}>Presión arterial</label><input style={ms.input} placeholder="120/80 mmHg" value={form.pressure} onChange={e => setForm(f => ({ ...f, pressure:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Peso</label><input style={ms.input} placeholder="70 kg" value={form.weight} onChange={e => setForm(f => ({ ...f, weight:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Temperatura</label><input style={ms.input} placeholder="36.5 °C" value={form.temperature} onChange={e => setForm(f => ({ ...f, temperature:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Pulso</label><input style={ms.input} placeholder="72 bpm" value={form.pulse} onChange={e => setForm(f => ({ ...f, pulse:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Talla</label><input style={ms.input} placeholder="1.70 m" value={form.height} onChange={e => setForm(f => ({ ...f, height:e.target.value }))} /></div>
          </div>
