...
export default function MedicationsSection() {
  ...
  const [form, setForm] = useState({ patient:"", medication:"", dose:"", frequency:"", duration:"" });

  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24 }}>
        <div><h2 style={sc.sectionTitle}>Medicamentos</h2><p style={sc.sectionSub}>Medicamentos recetados y seguimiento de alergias</p></div>
        <button style={sc.primaryBtn} onClick={() => setShowModal(true)}><SvgIcon name="plus" size={14} color="#fff" /> Agregar Medicamento</button>
      </div>
      <div style={sc.tableCard}>
        <table style={sc.table}>
          <thead><tr>{["Paciente","Medicamento","Dosis","Frecuencia","Duración","Acciones"].map(h => <th key={h} style={sc.th}>{h}</th>)}</tr></thead>
          <tbody>{[1,2,3,4].map(i => <tr key={i} style={sc.tr}>{[100,100,60,80,70,68].map((w,j) => <td key={j} style={sc.td}><div style={{ height:13, borderRadius:6, background:"rgba(30,58,138,0.07)", width:w }} /></td>)}</tr>)}</tbody>
        </table>
      </div>
      {showModal && (
        <div style={ms.backdrop}><div style={ms.modal}>
          <h3 style={ms.title}>Agregar Medicamento</h3>
          ...
          <div style={ms.field}><label style={ms.label}>Medicamento *</label><input style={ms.input} placeholder="Nombre del medicamento" value={form.medication} onChange={e => setForm(f => ({ ...f, medication:e.target.value }))} /></div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12 }}>
            <div style={ms.field}><label style={ms.label}>Dosis *</label><input style={ms.input} placeholder="500mg" value={form.dose} onChange={e => setForm(f => ({ ...f, dose:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Frecuencia *</label><input style={ms.input} placeholder="Cada 8h" value={form.frequency} onChange={e => setForm(f => ({ ...f, frequency:e.target.value }))} /></div>
            <div style={ms.field}><label style={ms.label}>Duración *</label><input style={ms.input} placeholder="7 días" value={form.duration} onChange={e => setForm(f => ({ ...f, duration:e.target.value }))} /></div>
          </div>
